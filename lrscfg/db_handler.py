...
class DB_Handler:
    ...
    def update_channels_table(self, column_names, data_types):
        self.cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='Channels'")
        ...
        if not table_exists:
            ...
            create_table_query += 'FOREIGN KEY (config_id) REFERENCES Configurations(config_id) )'
            self.cursor.execute(create_table_query)
        else:
            for i in range(len(column_names)):
                self.cursor.execute(f"PRAGMA table_info('Channels')")
                existing_columns = [row[1] for row in self.cursor.fetchall()]
                if column_names[i] not in existing_columns:
                    self.cursor.execute(f"ALTER TABLE Channels ADD COLUMN {column_names[i]} {data_types[i]}")
    ...
    def import_configuration(self, csv_file, tag):
        ...
        self.cursor.execute('SELECT version FROM Configurations WHERE version = ?', (version,))
        existing_version = self.cursor.fetchone()
        
        if not existing_version:
            self.cursor.execute('INSERT INTO Configurations (version,tag) VALUES (?,?)', (version,tag))
            ...
            with open(csv_file, 'r') as file:
                ...
                for row in csv_reader:
                    query = f'INSERT INTO Channels (config_id'
                    placeholder = '(?'
                    values = (config_id,)
    ...
    def get_configurations_by_version(self, version):
        self.cursor.execute('''
            SELECT Configurations.version, Channels.*
            FROM Configurations
            JOIN Channels ON Configurations.config_id = Channels.config_id
            WHERE Configurations.version = ?
        ''', (version,))
    ...
    def update_active_configuration(self, version):
        self.cursor.execute('UPDATE Configurations SET is_active = 0')
        self.cursor.execute('UPDATE Configurations SET is_active = 1 WHERE version = ?', (version,))
        self.conn.commit()

    def get_active_configuration(self):
        self.cursor.execute('SELECT version FROM Configurations WHERE is_active = 1')
